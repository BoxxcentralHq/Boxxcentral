...
export interface MembershipConfirmationEmail {
  memberName: string;
  memberEmail: string;
...
export class EmailService {
  ...
  private membershipConfirmationHtml(
    data: MembershipConfirmationEmail,
  ): string {
    const naira = (n: number) => `₦${n.toLocaleString('en-NG')}`;
      ...
      <div style="background:#f4f2ee;padding:32px 16px;${body}">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #e6e2da;border-radius:16px;overflow:hidden;${body}">
            ...
            <td style="padding:32px 32px 24px;border-bottom:2px solid #e4212e;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td align="right">
                    <p style="margin:0;font-size:26px;font-weight:normal;letter-spacing:0.04em;color:#111111;${heading}">MEMBERSHIP RECEIPT</p>
                    <p style="margin:2px 0 0;font-size:12px;color:#8a8a8a;">${formatDate(new Date())}</p>
                  </td>
                </tr>
              </table>
            </td>
            ...
            <td style="padding:24px 32px 0;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
                <tr style="background:#f4f2ee;">
                  ...
                  <td style="${label}" align="center">Valid through</td>
                  <td style="${label}padding-right:12px;border-radius:0 8px 8px 0;" align="right">Amount</td>
                </tr>
                <tr>
                  <td style="${cell}padding-left:12px;border-bottom:1px solid #eeece7;">
                  ...
                  <td style="${cell}border-bottom:1px solid #eeece7;" align="center">${formatDate(data.endDate)}</td>
          ...
          <tr>
            <td style="padding:28px 32px 32px;">
              <p style="margin:0;font-size:12px;line-height:1.6;color:#8a8a8a;">
                Your membership runs from ${formatDate(data.startDate)} to ${formatDate(data.endDate)}.
                This is a one-time pass, not a recurring charge — come back and renew any time
                after it lapses.
              </p>
            </td>
          </tr>
  ...
  async sendMembershipConfirmation(data: MembershipConfirmationEmail) {
    await this.send(
      { address: data.memberEmail, name: data.memberName },
      `Your GymBoxx membership ${data.subscriptionRef} is confirmed`,
      this.membershipConfirmationHtml(data),
    );
  }
}
