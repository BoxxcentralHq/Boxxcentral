...
- **Cross-site local dev**: if you run the frontend on `localhost` against a _deployed_
...
| `cinema_admin` | Movie catalog (CRUD + poster upload), bookings (view/cancel/complete). Sees a booking's payment _status_ but not the ledger.                               |
